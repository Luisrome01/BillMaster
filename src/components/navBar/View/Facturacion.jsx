...
const Facturacion = ({
  ...
  const [getValorNombre, setValorNombre] = useState(
    ClienteExterno ? ClienteExterno.name : ""
  );
  const [getValorDireccion, setValorDireccion] = useState(
    ClienteExterno ? ClienteExterno.direccion : ""
  );
  const [getValorRif, setValorRif] = useState(
    ClienteExterno ? ClienteExterno.rif : ""
  );
  ...
  const eliminarProducto = (codigo) => {
    const index = listProductos.findIndex(
  ...
  const addProduct = () => {
    fetch("/src/json/productos.json")
      ...
      .then((data) => {
        ...
        if (product) {
          const index = listProductos.findIndex(
            (element) => element.codigo === getCodigo
          );
          if (index !== -1) {
            ...
            updatedProductos[index].total =
              ...
              (parseFloat(updatedProductos[index].precio) +
                parseFloat(updatedProductos[index].iva));
            setListProductos(updatedProductos);
            setValorCodigo("");
            setValorCantidad("");
          } else {
  ...
  const handleOnBlur = () => {
    switch (getIdentificacion) {
      case "Cedula":
        const client = getClientes.find(
          (element) => element.ci === getValorIdentificacion
        );
        if (client && client.ci) {
        ...
        break;
      case "Pasaporte":
        const client2 = getClientes.find(
          (element) => element.pasaporte === getValorIdentificacion
        );
        ...
        break;
      case "ID Extranjero":
        const client3 = getClientes.find(
          (element) => element.idExtranjera === getValorIdentificacion
        );
      ...
      <div className="FacturaContainer">
        {message && (
          <div className="message-bar-wrapper">
            {message.severity === "success" &&
              showSuccessMessage(message.text, "left")}
            {message.severity === "error" &&
              showErrorMessage(message.text, "left")}
            {message.severity === "warning" &&
              showWarningMessage(message.text, "left")}
            {message.severity === "info" &&
              showInfoMessage(message.text, "left")}
          </div>
        )}
            ...
            <div className="FacturaRif-BotonCrear">
              ...
              <div className="FacturaBotonCrear">
                <BtnGeneral
                  ...
                  handleClick={() => {
                    createClient();
                    setUserCreated(true); // Set userCreated to true after creating the user
                  }}
                  disabled={userCreated} // Pass the userCreated state as disabled prop
                />
              </div>
            </div>
          </div>
        ...
        <div className="FacturaCheckoutContainer">
          ...
          <BtnGeneral
            ...
            disabled={!billCreated || !userCreated} // Disable if no bill is created or user is not created
          />
